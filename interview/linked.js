...
class LinkedList {
    ...
    append(value) {
     const newNode = {value, next: null, back: this.tail.value}
     this.tail.next = newNode
     this.tail = newNode
     this.length += 1;
     return this;
    ...
    prepend(value) {
      const newNode = {value, next: null, back: this.head.value}
      newNode.next = this.head
      this.head = newNode
      this.length +=1
      return this
    ...
    insert(index, value) {
        const newNode = {value, next: null, back: null}
        let stop = this.head
        let count = 0
        while (stop) {
            if(count === index) { break}
